...
from utils import amino_acids_from_1_nucleotide, amino_acids_from_2_nucleotides, amino_acid_from_3_nucleotides, codons_from_amino_acid
...
def optimize_recursive(gene_a, gene_b, constraints_a, constraints_b, offset):
    print("-"*100)
    ...
    for codon_a in codons_a:
        ...
        for codon_b in codons_b:
            if compare_codons(codon_a, codon_b, constraints_a, constraints_b, offset):
                ...
                sequence, errors = optimize_recursive(gene_a[3:], gene_b[3:], constraints_a, constraints_b, offset)
                if len(gene_a[3:]) == 0 and offset > 0:
                    sequence = codon_b[3-offset:] + sequence
                sequences.append((codon_a + sequence, errors))
                found_match = True
        if not found_match:
            print(f"no match found for {codon_a} and {codons_b} with offset {offset}")
            sequence, errors = optimize_recursive(gene_a[3:], gene_b[3:], None, None, offset)
            if len(gene_a[3:]) == 0 and offset > 0:
                sequence = codon_b[3-offset:] + sequence
            sequences.append((codon_a + sequence, errors + 1))

    print(f"sequences: {sequences}")
    return min(sequences, key=lambda x: (x[1], x[0][:3] != nucleotides_a))
    

def optimize_protein_sequence(dna_sequence, gene_a_start, gene_a_end, gene_b_start, gene_b_end, new_gene_b):
